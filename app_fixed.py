...
def fetch_1month_sector_trends():
    ...
    for sector, symbol in sector_etfs.items():
        ...
        try:
            ...
            start_price = int(first_day[4])
            end_price = int(last_day[4])
            return_rate = (end_price - start_price) / start_price * 100
            results.append({"업종/테마": sector, "최근1달수익률(%)": return_rate, "변동": f"+{return_rate:.2f}%" if return_rate > 0 else f"{return_rate:.2f}%"})
        except Exception:
            continue
    ...
    try:
        ...
        if tables:
            ...
            for tr in tb.select('tbody tr'):
                ...
                if th:
                    if '영업이익률' in th.text:
                        op_margin = tr.select_one('td').text.strip() if tr.select_one('td') else "0"
                    elif 'ROE' in th.text:
                        roe = tr.select_one('td').text.strip() if tr.select_one('td') else "0"
    ...
    try:
        ...
        for row in rows:
            ...
            if len(tds) >= 3:
                ...
                if len(code_text) >= 5 and code_text.isdigit():
                    ...
                    if not fundamentals:
                        ...
                        if tech is None:
                            ...
                            if tech['price'] > tech['sma20'] and tech['macd'] > tech['signal']:
                                ...
                                tech_score = 8
                            mgmt_score = 0
                            if fundamentals['op_margin'] >= 10:
                                mgmt_score += 12
                            elif fundamentals['op_margin'] >= 5:
                                mgmt_score += 8
                            else:
                                mgmt_score += 4
                            ...
                            if 0 < per < 15:
                                ...
                                val_score += 3
                            if 0 < fundamentals['pbr'] < 1.5:
                                val_score += 4
                            elif 1.5 <= fundamentals['pbr'] < 3:
                                val_score += 2
                            ...
                            with t2:
                                ...
                                st.write(f"- **MACD**: {_safe_float(tech.get('macd'))}")
                                st.write(f"- **Signal**: {_safe_float(tech.get('signal'))}")
                            with t3:
                                if news:
                                    for n in news[:5]:
